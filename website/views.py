    ...
    if current_user.is_super_admin == True:
        if request.method == 'POST':
            ...
            if len(movie_title) >= 1 and len(movie_starring) >= 1 and len(movie_production_house) >= 1:
                # good to go
                new_movie = Movie(poster=movie_poster, title=movie_title, starring=movie_starring,
                                  production_house=movie_production_house, no_of_watched=movie_no_of_watched, movie_admin_id=current_user.id)
...
def update_movies(id):
    if current_user.is_super_admin == True:
        ...
        if request.method == 'POST':
            ...
            movie_to_update.production_house = request.form.get(
                'production_house')

            db.session.commit()

            flash(movie_to_update.title +
                  ' updated succesfully', category='success')
            return redirect(url_for('views.movies'))
        else:
            if movie_to_update:
...
def add_theaters():
    if current_user.is_theatre_admin == True:
        if request.method == 'POST':
            ...
            # check if all the fields are not empty
            if len(theater_name) >= 1 and len(theater_address) >= 1 and len(theater_location) >= 1 and len(theater_city) >= 1:
                # good to go
                new_theater = Theaters(name=theater_name, address=theater_address, location=theater_location,
                                       city=theater_city, contact_no=theater_contact_no, theater_admin_id=current_user.id)
...
def book_ticket(id):
    if current_user.is_super_admin == False and current_user.is_theatre_admin == False:
        ...
        if request.method == 'POST':
            theater = Theaters.query.filter_by(
                name=str(show_to_book.theater)).first()

            print(theater.name)
            ticket_no_of_seats = request.form.get('no_of_seats')
            # update the number of seats
            show_to_book.seats_available = int(
                show_to_book.seats_available) - int(ticket_no_of_seats)

            new_ticket = Ticket(show_booked=show_to_book.id, movie_booked=show_to_book.movie_screened, theater_booked=show_to_book.theater_screened_in, user=current_user.id, movie_name=show_to_book.movie, theater_name=show_to_book.theater,
                                theater_address=theater.address, theater_address_link=theater.location, no_of_seats=ticket_no_of_seats, total_cost=int(show_to_book.cost_per_seat) * int(ticket_no_of_seats), show_timinig=show_to_book.datetime_screened)
...
def add_shows():
    print('reached 116')
    movies = Movie.query.all()
    theaters = Theaters.query.filter_by(
        theater_admin_id=int(current_user.id)).all()
    if current_user.is_theatre_admin == True:
        print('reached 120')
        if request.method == 'POST':
            print('reached 122')
            show_movie_screened = Movie.query.filter_by(
                title=request.form.get('movie_screened')).first().id
            if show_movie_screened:
                show_theater_screened_in = request.form.get(
                    'theater_screened_in')
                show_movie = request.form.get('movie_screened')
                show_theater = Theaters.query.filter_by(
                    id=request.form.get('theater_screened_in')).first()
                show_date_time_screened = datetime.strptime(
                    request.form.get('date_time_screened'), '%Y-%m-%dT%H:%M')
            ...
            if show_movie_screened >= 1 and int(show_seats_available) >= 1:
                new_show = Show(movie_screened=show_movie_screened, theater_screened_in=show_theater_screened_in, movie=show_movie, theater=show_theater.name, theater_address=show_theater.address,
                                theater_address_link=show_theater.location, datetime_screened=show_date_time_screened, theater_admin_id=current_user.id, seats_available=show_seats_available, cost_per_seat=show_cost_per_seat)
...
def save_images(poster):
    ...
    file_path = os.path.join(current_app.root_path,
                             'static/movie_posters', image_name)
